  ...
  if (path === "/api/state" && method === "GET") {
    ...
    if (history.length) {
      try {
        ...
        const delRes = await delStmt.bind.apply(delStmt, ids).all();
        deliveries = delRes.results || [];
      } catch (e) {
        await log(env, "warn", "خواندن جزئیات ارسال‌ها ناموفق بود", { error: String((e && e.message) || e) });
      }
    }
...
const STATIC_BASES = [
  "https://cdn.jsdelivr.net/gh/0xzare/aya-of-day@main/public/",
  "https://raw.githubusercontent.com/0xzare/aya-of-day/main/public/"
];
const STATIC_TYPES = {
